...
double **allocateDblMatrix(unsigned n_rows, unsigned n_cols) {
	...
	unsigned i, j;
	CPHYPROB_DEBUG_PRINTF1("In allocateDblMatrix len = %d \n", n_rows*n_cols);
	assert(n_rows > 0);
	assert(n_cols > 0);
...
double ***allocateDbl3DMatrix(unsigned nm, unsigned nr, unsigned nc) {
	...
	unsigned i, j, k;
	CPHYPROB_DEBUG_PRINTF1("In allocateDbl3DMatrix len =%d\n", arr_len);
	assert(nm > 0);
	assert(nr > 0);
	assert(nc > 0);
...
void freeDblMatrix(double **p) {
	CPHYPROB_DEBUG_PRINTF("In freeDblMatrix\n");
	if (p == 0L)
		return;
	if (p[0] != 0L)
		free(p[0]);
...
void freeDbl3DMatrix(double ***p) {
	CPHYPROB_DEBUG_PRINTF("In freeDbl3DMatrix\n");
	if (p == 0L)
		return;
	if (p[0] != 0L) {
		if (p[0][0] != 0L) {
